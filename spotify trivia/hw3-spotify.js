 ...
 * spotify game that allow the user to interact with the
 * system by searching for artists on spotify and getting 
 * their top tracks, so that they can arrange them and get a score.
 * 
 * A late submission for this homework was approved by El via email.
 */
(function() {
    "use strict";
    ...
    const CLIENT_ID = "0dbae37cebf0477f83b2b3692abbaf5c";
    const CLIENT_SECRET = "8ef57c774ddc4caab7556fb80273c1f1";
    ...
    async function init() {
        ...
        qs("#play-view-done").addEventListener("click", () => {
            populateResults();
            showSection("results-view");
        });

        qs("#results-view-done").addEventListener("click", () => {
    ...
    async function fetchArtists(name) {
        ...
        let url = SEARCH_EP + "&type=artist" + `&limit=${MAX_SEARCH_RESULTS}` + `&q=${name}`;
        fetch(url , {
            headers : {
    ...
    function populateArtistResults(artistsData) {
        ...
        if (artistsData.artists.items.length === 0){
            ...
            items.forEach(item => {
                let article = genArtistCard(item);
                qs("#search-results").appendChild(article);
            })
        }
    }
    ...
    function genArtistCard(artistInfo) {
        ...
        h3.textContent = artistInfo.name;
    ...
    */
    function populateTracks() {
        qs("#track-container").innerHTML = '';
        sortTracksByGameIdx();
        tracks.forEach(track => {
            let card = genTrackCard(track);
            qs("#track-container").appendChild(card);
        })
    }
    ...
    function populateResults() {
        ...
        const actual = qs("#actual-ordering");
        actual.innerHTML = "";
